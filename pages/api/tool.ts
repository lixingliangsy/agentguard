...
import { runDeterministicChecks } from '../../lib/rules/agent-guardrails'
...
function formatReport(state: {
  ...
  lines.push('')
  lines.push('=== Model-assisted notes ===')
  lines.push(modelText || '(demo / unavailable)')
  lines.push('')
  lines.push('---')
  lines.push('Sources: Rule-based checklist + Model-assisted narrative (when live AI is configured).')
  return lines.join('\n')
}
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const requested = (body.step || 'ingest') as StepId
    if (!STEP_ORDER.includes(requested)) {
      return res.status(400).json({ error: 'Unknown step', code: 'INVALID_STEP' })
    }

    // Allow start at ingest; otherwise must advance sequentially
    if (body.runId) {
    ...
    } else if (requested !== 'ingest') {
      return res.status(400).json({
        error: 'New runs must start at ingest',
        code: 'INVALID_TRANSITION',
      })
    }
    ...
    if (state.step === 'ingest') {
      ...
      // Auto-advance path for single-shot clients: continue to analyze+report in one request
      state = advance(state, 'analyze')
    }
