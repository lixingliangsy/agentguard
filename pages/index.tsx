  ...
  async function runStudio(e?: FormEvent) {
    ...
    try {
      ...
      const r = await fetch('/api/tool', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ useMock: useDemoMode, inputs: inputMap, action: 'start', step: 'ingest' }),
      })
      ...
      setResult(String(data.result || JSON.stringify(data, null, 2)))
  ...
  return (
    <>
      <Head>
        ...
        <link rel="canonical" href="https://agentguard.lxsaihub.com/" />
        <script type="application/ld+json">{`{"@context": "https://schema.org", "@type": "FAQPage", "mainEntity": [{"@type": "Question", "name": "What risks does AgentAegis catch?", "acceptedAnswer": {"@type": "Answer", "text": "Secret leakage in prompts, prompt-injection patterns, and unsafe tool or function calls, mapped to the OWASP Top 10 for LLM Applications."}}, {"@type": "Question", "name": "Does it run live AI?", "acceptedAnswer": {"@type": "Answer", "text": "Pro uses the platform LLM key under fair use; Free and demo mode return a realistic mock report without live calls."}}, {"@type": "Question", "name": "How does it map to OWASP LLM Top 10?", "acceptedAnswer": {"@type": "Answer", "text": "Findings reference LLM01 (prompt injection), LLM02 (data exfiltration), and LLM06 (over-reliance) where relevant."}}, {"@type": "Question", "name": "Can I export the guardrail report?", "acceptedAnswer": {"@type": "Answer", "text": "Yes - Pro exports a report with runId and ruleset version for audit."}}, {"@type": "Question", "name": "Is it a WAF?", "acceptedAnswer": {"@type": "Answer", "text": "No. It is a pre-deployment review layer for agent builders, not a network firewall."}}, {"@type": "Question", "name": "Which agent frameworks are supported?", "acceptedAnswer": {"@type": "Answer", "text": "Any agent that exposes a system prompt and tool list (LangChain, AutoGen, or a custom stack)."}}]}`}</script>
      ...
      <div className="min-h-screen bg-white text-slate-900">
        ...
        <section id="definition" className="py-20 bg-slate-50">
          <div className="max-w-6xl mx-auto px-6">
            ...
            <h2 className="text-3xl md:text-4xl font-extrabold mb-4">AgentAegis - definition</h2>
        ...
        <section id="how" className="py-20 bg-slate-50">
          <div className="max-w-6xl mx-auto px-6">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {[
                ['1', 'Ingest', 'Paste system prompt + tool list.'],
                ['2', 'Analyze', 'Rule-based risk scoring, then model-assisted filters.'],
                ['3', 'Report', 'Export guardrail report with runId + ruleset version.'],
              ].map(([n, t, d]) => (
        ...
        <section id="studio" className="py-20">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <form onSubmit={runStudio} className="rounded-2xl border border-slate-200 p-6 bg-white shadow-sm">
                ...
                <button type="submit" disabled={toolBusy} className="mt-4 w-full py-3 rounded-full bg-indigo-600 text-white font-bold disabled:opacity-60">
                  {toolBusy ? 'Running workflow…' : (PRODUCT as any).ctaLabel || 'Run workflow'}
                </button>
                {toolStatus ? <p className="mt-3 text-sm text-emerald-600">{toolStatus}</p> : null}
                {runId ? <p className="mt-1 text-xs text-slate-500">runId: {runId}</p> : null}
              </form>
              <div className="rounded-2xl border border-slate-200 p-6 bg-white shadow-sm">
                <h3 className="font-bold mb-4">Result</h3>
                {demoMode ? (
                  <div className="mb-3 rounded-lg border border-amber-300 bg-amber-50 text-amber-900 text-sm px-3 py-2 font-semibold">
                    Demo mode — not live AI
                  </div>
                ) : null}
                ...
                <div className="flex flex-wrap gap-3 mt-4">
                  ...
                  <a href="/api/runs?export=json" className="px-4 py-2 rounded-full border border-slate-200 font-semibold">
                    Export runs (JSON)
                  </a>
                  <button
                    type="button"
                    className="px-4 py-2 rounded-full border border-slate-200 font-semibold"
                    onClick={async () => {
                      if (!runId) return showToast('Run a workflow first')
                      await fetch('/api/feedback', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ runId, thumbs: 'up' }),
                      })
          ...
          <div className="max-w-6xl mx-auto px-6">
            <h2 className="text-3xl md:text-4xl font-extrabold text-center mb-4">Trusted by agent platform teams</h2>
            <p className="text-center text-slate-600 mb-12 max-w-2xl mx-auto">
              Social proof uses templates until real, consented testimonials exist. Trusted by <strong>[X]+</strong> agent builders and compliance leads.
            </p>
            ...
            <div className="grid md:grid-cols-3 gap-6">
              <div className="rounded-2xl border border-slate-200 p-6">
                ...
                <ul className="space-y-2 text-sm text-slate-600 mb-6">
                  <li className="flex gap-2"><Check /> 1 workflow run / day · watermarked export</li>
              ...
              <div className="rounded-2xl border-2 border-indigo-600 p-6 relative shadow-lg">
                ...
                <ul className="space-y-2 text-sm text-slate-600 mb-6">
                  <li className="flex gap-2"><Check /> 300 workflow runs / mo · audit log · fair-use cost guard</li>
                  <li className="flex gap-2"><Check /> No separate ChatGPT subscription required</li>
                  <li className="flex gap-2"><Check /> Priority support · 2 months free with yearly</li>
                </ul>
                ...
                <a href="/api/checkout?cycle=yearly" onClick={(e) => goCheckout('/api/checkout?cycle=yearly', e)} className="block text-center mt-3 text-sm font-semibold text-indigo-600">
                  Or pay yearly — ${priceYearlyMonthly}/mo
                </a>
              </div>
              <div className="rounded-2xl border border-slate-200 p-6">
        ...
        <section id="product-faq" className="py-20">
          <div className="max-w-3xl mx-auto px-6">
            <h2 className="text-3xl font-extrabold text-center mb-10">AgentAegis - frequently asked questions</h2>
            ...
            <div className="space-y-3">
              {[
                ...
                [
                  ...
                  'Pro includes about 300 workflow runs/month (fair use cost guard ≈ 300 gens on gpt-4o-mini, plus a daily cap). If you hit the fair-use limit, Studio returns HTTP 429 — upgrade, wait for reset, enable Demo mode, or use Enterprise BYOK. We never silently return fake AI success.',
                ],
