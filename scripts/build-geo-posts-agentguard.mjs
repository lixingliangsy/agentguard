// build-geo-posts-agentguard.mjs
// Reads GEO article markdown files from opc-doc (agentguard), extracts frontmatter + JSON-LD +
...
const ARTICLES_DIR = 'E:/AgentCPM/07_一人公司出海项目/opc-doc/outputs/07-conversion/articles-agentguard'
const OUT_TS = 'E:/AgentCPM/07_一人公司出海项目/12_Micro_SaaS出海/agentguard/data/geoPosts.ts'
// NOTE: Vercel alias is NOT agentguard.lxsaihub.com — after first deploy, replace both with the
// real alias (e.g. agentguard-<random>.vercel.app) and re-run this script + redeploy.
const PROD_URL = 'https://agentguard-liart.vercel.app'
const LIVE_DOMAIN = 'https://agentguard-liart.vercel.app'
...
function parseFrontmatter(md) {
  // Preferred: Owner line, blank line, --- fenced frontmatter --- (matches privacyscan/agentredteam samples)
  ...
  // Tolerant fallback: Owner line, then key: value lines until blank line or a Markdown heading.
  // Handles articles that omit the --- fences (e.g. agentguard sample) so slug is still extracted.
  const lines = md.split('\n')
  let i = 0
...
function parseCta(rest) {
  ...
  if (!line) return ''
  const text = line.replace(/\[WAITLIST_URL\]/g, `<a href="${PROD_URL}" class="underline font-semibold">try AgentAegis</a>`)
  return '<p>' + inline(text) + '</p>'
}

function fixDomain(obj) {
  return JSON.parse(JSON.stringify(obj).replace(/https:\/\/agentguard\.com/g, LIVE_DOMAIN))
}
...
for (const f of files) {
  ...
  const description = (blogPosting.description || '').replace(/https:\/\/agentguard\.com/g, LIVE_DOMAIN)
  posts.push({
    slug: fm.slug,
    ...
    description,
    html: mdToHtml(body).replace(/https:\/\/agentguard\.com/g, LIVE_DOMAIN),
    faq: parseFaq(faq),
...
const ts = `// AUTO-GENERATED by scripts/build-geo-posts-agentguard.mjs — do not edit by hand.
